...
# V3.1 Addendum — Context Management as First-Class Module

## 24. Context Management UX Requirement

Context Management must be treated as a core product workflow, not as a simple settings form. Organization admins need dedicated screens to define, inspect, version, compare, and simulate the impact of context rules.

The app must support the following context management screens:

1. Context Dashboard — overview of all active rule layers.
2. Organization Context — global rules for all tournaments.
3. Tournament Context — tournament-specific overrides.
4. Category Context — category-specific start date, game mode, duration, priority, and restrictions.
5. Date Context — date-specific restrictions and overrides.
6. Context Version History — historical list of context versions.
7. Context Compare — diff between two context versions.
8. Context Impact Simulator — preview affected matches/categories/dates before full dry run.

## 25. Required Context Screen Behavior

Every context editor must include:

- Current active rules.
- Inherited rules where applicable.
- Prompt input in Spanish or English.
- Prompt examples.
...
- Generate Dry Run action where fixture-affecting changes exist.

## 26. Category Context Eligibility UX

Category Context must clearly show whether a category is eligible for fixture generation.

A category is fixture-eligible only when it has:

1. Start date.
2. Game mode.

If either is missing, the UI must show a clear warning and keep the category inactive for fixture generation.

## 27. Context Impact Simulator

The Context Impact Simulator provides a fast preview before a full dry run. It should show:

- Affected fixture versions.
- Affected categories.
- Affected dates.
- Estimated moved matches.
- Estimated added/removed matches.
- Manual overrides preserved.
- Locked past matches unchanged.
- Warnings and conflicts.

The simulator does not replace dry run approval. A dry run remains mandatory before committing fixture changes.
