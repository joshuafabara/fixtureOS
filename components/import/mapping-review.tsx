...
import { ChevronLeft, ChevronRight, CheckCircle2, AlertTriangle, Eye, List, Loader2 } from "lucide-react";
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            {isImage ? "Datos extraídos" : "Filas de previsualización"}
            <span className="text-xs font-normal text-muted-foreground ml-1">
              {isImage ? "Confianza por fila" : "Primeras filas tras aplicar el mapeo"}
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <div className={`min-w-[640px]`}>
              <div className={`grid ${isImage ? "grid-cols-[140px_1.1fr_1.3fr_90px_110px_110px]" : "grid-cols-[1.2fr_1.3fr_100px_100px_110px_110px]"} px-4 py-2.5 text-[11px] font-extrabold uppercase tracking-widest text-muted-foreground border-b border-zinc-100`}>
                {isImage
                  ? <><div>Confianza</div><div>Club</div><div>Equipo</div><div>Cat.</div><div>Estado</div><div className="text-right">Marca</div></>
                  : <><div>Club</div><div>Equipo</div><div>Categoría</div><div>Color</div><div>Estado</div><div className="text-right">Marca</div></>}
              </div>
              {isImage
                ...
                : preview.rows.slice(0, 15).map((r, i, arr) => (
                  <div key={i} className={`grid grid-cols-[1.2fr_1.3fr_100px_100px_110px_110px] items-center px-4 py-3 ${i < arr.length - 1 ? "border-b border-zinc-100" : ""}`}>
                    <div className="font-bold text-[13.5px] truncate">{r.clubName}</div>
                    <div className="text-[13px] text-muted-foreground truncate">{r.teamName}</div>
                    <div className="text-[12.5px] font-semibold truncate">{r.categoryName}</div>
                    <div className="text-[12.5px] text-muted-foreground">{r.categoryColor ?? "—"}</div>
                    <div className="text-[12.5px] text-muted-foreground">Activo</div>
                    <div className="text-right"><FlagTag flag={null} /></div>
                  </div>
                ))}
            </div>
          </div>
